...
class BettingModelTests(unittest.TestCase):
    ...
    def test_publish_telegram_predictions_usa_fallback_de_emergencia_si_el_operativo_no_entra(self):
        sent = []
        result = publish_telegram_predictions(
            runtime_settings=RuntimeSettings(environment="production", shadow_mode=False),
            publication_guard=lambda: {"allow_live_publication": True, "mode": "live", "reasons": []},
            ...
            save_unique_recommendations=lambda picks: [{"id": 299, **picks[0]}],
            read_raw_pick=lambda pick: pick,
            enrich_with_ai=lambda picks: picks,
            build_ai_summary=lambda *args, **kwargs: None,
            ai_available=lambda: False,
            ...
            telegram_keyboard_for_pick=lambda pick_id: {"inline_keyboard": [[{"text": "Apostada", "callback_data": f"pick:{pick_id}:bet"}]]},
            send_message=lambda text, token=None, chat_id=None, reply_markup=None: sent.append((text, reply_markup)) or {"ok": True, "result": {"message_id": len(sent)}},
            register_publication=lambda publication_type, payload, items: {"id": 655, "items": items},
            perfil_label=lambda value: value or "agresivo",
            modo_label=lambda value: value or "comparador",
            perfiles_stake={"agresivo"},
            modos_informe={"comparador"},
    ...
    def test_publish_telegram_predictions_acepta_motivo_promocionable_en_espanol(self):
        sent = []
        result = publish_telegram_predictions(
            runtime_settings=RuntimeSettings(environment="production", shadow_mode=False),
            publication_guard=lambda: {"allow_live_publication": True, "mode": "live", "reasons": []},
            ...
            save_unique_recommendations=lambda picks: [{"id": 399, **picks[0]}],
            read_raw_pick=lambda pick: pick,
            enrich_with_ai=lambda picks: picks,
            build_ai_summary=lambda *args, **kwargs: None,
            ai_available=lambda: False,
            ...
            telegram_keyboard_for_pick=lambda pick_id: {"inline_keyboard": [[{"text": "Apostada", "callback_data": f"pick:{pick_id}:bet"}]]},
            send_message=lambda text, token=None, chat_id=None, reply_markup=None: sent.append((text, reply_markup)) or {"ok": True, "result": {"message_id": len(sent)}},
            register_publication=lambda publication_type, payload, items: {"id": 656, "items": items},
            perfil_label=lambda value: value or "agresivo",
            modo_label=lambda value: value or "comparador",
            perfiles_stake={"agresivo"},
            modos_informe={"comparador"},
    ...
    def test_seleccionar_picks_para_apuestas_lab_acepta_premium_si_no_hay_elite(self):
        import main
        from datetime import datetime, timezone
                    ...
                    "markets": [
                        {
                            "key": "totals",
                            "outcomes": [
                                {"name": "Under", "point": 3.0, "price": 1.96},
                            ],
                        },
                        {
                            "key": "alternate_totals",
                            "outcomes": [
                                {"name": "Under", "point": 4.0, "price": 1.57},
                            ],
                        },
                    ],
                }
            ...
            picks = main.seleccionar_picks_para_apuestas_lab(
                {
                    ...
                    "_event_odds_map": {"evt-4": raw_event},
                }
            )
        finally:
            main.datetime = original_datetime
